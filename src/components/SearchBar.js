...
const SearchBar = memo(() => {
  ...
  const hits = useMemo(() => {
    ...
    return grades.filter(g =>
      g.subject.toLowerCase().includes(ql) || g.type.toLowerCase().includes(ql) || String(g.grade).includes(ql)
    ).slice(0, 7);
  }, [q, grades]);
  ...
  return (
    <div ref={ref} style={{ position:"relative", width:260 }}>
      <div style={{ position:"relative" }}>
        <svg style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }} width={13} height={13} viewBox="0 0 24 24" fill="none" stroke={C.t2} strokeWidth={2} strokeLinecap="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
...
/>
        {q && <button onClick={()=>{setQ("");setIsOpen(false);}} style={{position:"absolute",right:9,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:C.t2,fontSize:13}}>✕</button>}
      </div>
      <AnimatePresence>
          ...
          <motion.div initial={{opacity:0,y:-6}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6}} transition={{duration:0.12}}
            ...
            {hits.map(g=>(
              <button key={g.id} onMouseDown={()=>{setQ("");setIsOpen(false);}}
                style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:"none",border:"none",cursor:"pointer",fontFamily:"inherit",transition:"background 0.1s"}}
                onMouseEnter={e=>(e.currentTarget.style.background=C.bg4)} onMouseLeave={e=>(e.currentTarget.style.background="none")}>
                <div><div style={{fontSize:13,fontWeight:500,color:C.t0}}>{g.subject}</div><div style={{fontSize:10,color:C.t2,marginTop:1}}>{g.type} · {fDE(g.date)}</div></div>
