        ...
        {open && (
          <motion.div
            ...
            style={{
              ...
              background:C.bg3, border:`1px solid ${C.lineH}`, borderRadius:R.m,
              boxShadow:"0 16px 50px rgba(0,0,0,0.8)", overflow:"hidden", maxHeight:220, overflowY:"auto",
            }}
          >
            {/* No results + show create */}
            {filtered.length === 0 && showCreate && (
              <button
                onMouseDown={() => sel(q.trim())}
            ...
            {filtered.map((opt, i) => (
              <button
                ...
                style={{
                  ...
                  background: i === hi ? C.bg4 : "none",
                  border:"none", cursor:"pointer",
                  color: i === hi ? C.t0 : C.t1,
            ...
            {/* Create option at bottom — only when typed value doesn't exist exactly */}
  ...
  const handleManual = e => {
    ...
    if (m) {
      ...
      if (!isNaN(dt)) {
        ...
        onChange(ds); setYr(+y2); setMo(+m2-1); setMErr(""); return;
      }
    }
