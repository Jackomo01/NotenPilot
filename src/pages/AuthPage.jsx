...
const AuthPage = memo(({ onAuth }) => {
  ...
  return (
    <div ref={pageRef} style={{ minHeight:"100vh", background:C.bg0, display:"flex", alignItems:"center", justifyContent:"center", padding:16, position:"relative", overflow:"hidden" }}>
      <div style={{ position:"fixed", inset:0, pointerEvents:"none", backgroundImage:`linear-gradient(${C.line} 1px,transparent 1px),linear-gradient(90deg,${C.line} 1px,transparent 1px)`, backgroundSize:"44px 44px", opacity:0.3, maskImage:"radial-gradient(ellipse 60% 60% at 50% 50%,#000 30%,transparent 100%)" }}/>
      <div style={{ position:"fixed", top:"20%", left:"50%", transform:"translate(-50%,-50%)", width:400, height:400, background:C.accGlow, borderRadius:"50%", filter:"blur(80px)", pointerEvents:"none" }}/>
      ...
      <div style={{ position:"relative", zIndex:1, width:"100%", maxWidth:400 }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          ...
          <p style={{ fontSize:13, color:C.t1 }}>{mode==="login" ? "Melde dich an, um fortzufahren." : "Starte kostenlos durch."}</p>
        </div>

        <Card pad="28px 32px">
          <ClickSpark sparkColor="#4285F4" sparkCount={8} sparkRadius={22} sparkSize={9} duration={420}>
            <motion.button onClick={googleAuth} disabled={loading} whileTap={{ scale:0.97 }}
              style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"center", gap:10, padding:"10px 18px", background:"#fff", color:"#111", border:"1px solid #e0e0e0", borderRadius:R.s, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit", transition:"background 0.12s" }}
          ...
          {mode==="register"&&<div style={{marginBottom:14}}><Lbl>Name</Lbl><TxtInp value={name} onChange={e=>setName(e.target.value)} placeholder="Dein Name"/></div>}
          <div style={{marginBottom:14}}><Lbl>E-Mail</Lbl><TxtInp value={email} onChange={e=>setEmail(e.target.value)} placeholder="name@schule.de" autoFocus={mode==="login"}/></div>
          ...
          <div style={{marginTop:18}}>
            <SparkBtn full disabled={loading} onClick={submit}>
              {loading ? <span style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                Bitte warten…
              </span> : mode==="login"?"Anmelden":"Konto erstellen"}
            </SparkBtn>
          </div>
          <div style={{textAlign:"center",marginTop:16,fontSize:13,color:C.t1}}>
            {mode==="login"?"Noch kein Konto? ":"Bereits registriert? "}
            <motion.button onClick={()=>{setMode(mode==="login"?"register":"login");setErr("");}} whileHover={{color:C.accH}}
