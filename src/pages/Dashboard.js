...
const Dashboard = memo(({ loading, user }) => {
  ...
  const sStats = useMemo(() =>
    subjects.map(s=>{const sg=grades.filter(g=>g.subject===s);return{name:s,avg:wAvg(sg),count:sg.length};})
  ...
  const best=sStats[0], worst=sStats.length>1?sStats[sStats.length-1]:null;
  ...
  return (
    <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.3 }} style={{ display:"grid", gap:16 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
        {[
          [()=><><Lbl>Gesamtdurchschnitt</Lbl><div style={{fontSize:50,fontWeight:900,letterSpacing:"-0.06em",color:avg?gc(avg):C.t2,lineHeight:1,marginBottom:4}}>{avg?animAvg.toFixed(2):"–"}</div><div style={{fontSize:12,color:C.t1}}>{avg?gl(avg):"Keine Noten"}</div></>, {position:"relative",overflow:"hidden"}],
          [()=><><Lbl>Noten gesamt</Lbl><div style={{fontSize:42,fontWeight:800,letterSpacing:"-0.05em",color:C.t0,lineHeight:1,marginBottom:4}}><AnimNum to={grades.length}/></div><div style={{fontSize:12,color:C.t1}}>{subjects.length} Fächer</div></>, {}],
          [()=><><Lbl>Bestes Fach</Lbl>{best?<><div style={{fontSize:15,fontWeight:600,color:C.t0,marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{best.name}</div><div style={{fontSize:38,fontWeight:800,letterSpacing:"-0.05em",color:gc(best.avg),lineHeight:1}}>{best.avg.toFixed(2)}</div></>:<div style={{fontSize:13,color:C.t2,marginTop:8}}>–</div>}</>, {}],
          [()=><><Lbl>Schwächstes Fach</Lbl>{worst?<><div style={{fontSize:15,fontWeight:600,color:C.t0,marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{worst.name}</div><div style={{fontSize:38,fontWeight:800,letterSpacing:"-0.05em",color:gc(worst.avg),lineHeight:1}}>{worst.avg.toFixed(2)}</div></>:<div style={{fontSize:13,color:C.t2,marginTop:8}}>–</div>}</>, {}],
        ].map(([Render, st], i) => (
          <Card key={i} pad="22px 24px" style={st}>
            {i===0 && <div style={{ position:"absolute", inset:0, background:`radial-gradient(ellipse at top right,${avg?gc(avg):C.acc}14 0%,transparent 70%)`, pointerEvents:"none" }}/>}
            <Render/>
          </Card>
        ))}
      </div>
          ...
          {trend.length>=2 ? (
            <ResponsiveContainer width="100%" height={210}>
              <AreaChart data={trend} margin={{top:4,right:4,bottom:0,left:-22}}>
                <defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.acc} stopOpacity={0.3}/><stop offset="100%" stopColor={C.acc} stopOpacity={0}/></linearGradient></defs>
                ...
                <Area type="monotone" dataKey="Schnitt" stroke={C.acc} strokeWidth={2.5} fill="url(#ag)" dot={false} name="Durchschnitt"/>
                <Line type="monotone" dataKey="Note" stroke={C.lineH} strokeWidth={1} dot={{fill:C.acc,r:3.5,strokeWidth:0}} name="Note"/>
              </AreaChart>
            </ResponsiveContainer>
          ) : <div style={{height:210,display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:C.t2}}>Mind. 2 Noten für den Verlauf</div>}
        </Card>
        <Card pad="22px">
          <div style={{ fontSize:13, fontWeight:600, color:C.t0, marginBottom:14 }}>Zuletzt eingetragen</div>
          {recent.length===0 ? <div style={{fontSize:13,color:C.t2}}>Noch keine Noten.</div>
          : recent.map((g,i) => (
